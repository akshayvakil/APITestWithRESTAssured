...
// Send POST request and capture the response
		Response response = given().contentType(ContentType.JSON).body(requestBody).when().post("/register");
...
// For example, to assert the response body contains the expected values
	...
	public Object[][] getInvalidData() {
		return new Object[][] { { "", "pistol" }, // Empty email
				{ "eve.holt@reqres.in", "" }, // Empty password
				{ "invalidemail", "pistol" }, // Invalid email format
				{ "eve.holt@reqres.in", "short" } // Short password
		};
	}

	@Test(description = "Test negative scenarios for POST method withinvalid data", dataProvider = "invalidData")
