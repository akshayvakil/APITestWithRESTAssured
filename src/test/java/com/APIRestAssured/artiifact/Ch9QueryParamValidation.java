...
public class Ch9QueryParamValidation {
    ...
    public Object[][] pageNumbers() {
        return new Object[][]{
                {1},   // Minimum valid page number
                {5},   // Valid page number within the range
                {10},  // Maximum valid page number
                {0},   // Invalid page number (below minimum)
                {11},  // Invalid page number (above maximum) //This is test data so it gives 400 but does not fail
    ...
    public void testGetUserList(int pageNumber) {
        ...
        if (pageNumber >= 1 && pageNumber <= 10) {
            // Expecting a 200 OK status code for valid page numbers within the range [1, 10]
